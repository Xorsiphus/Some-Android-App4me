...
import { StyleSheet, SafeAreaView, FlatList, TouchableOpacity, StatusBar, Text, Image, Dimensions, Linking, View } from 'react-native';
...
function FavoritesRecipesScreen(props) {
    ...
    const reloadDb = () => db.transaction((tx) => {
        // tx.executeSql("drop table recipes", []);     
        tx.executeSql("select * from recipes", [], (_, { rows }) => setRecipes(rows['_array']), (_, { err }) => err);     
    });
                    ...
                    return (
                        <View>
                            <TouchableOpacity 
                                ...
                                <Text style={styles.ingredients}>{'➨ ' + item.ingredientLines.replace(/,/g, '\n➨ ') || 'Не указано'}</Text>
                            </TouchableOpacity>
